...
static void SB1_ResetGlobalStuff()
{
    ...
    // Reset new fight flow fields
    sSB1_Ptr->m_rumbleStompTimer = 0.0f;
    sSB1_Ptr->m_flexing = false;
    sSB1_Ptr->m_loopCount = 0;
}
...
static S32 rumbleStompLoopCB(xGoal* rawgoal, void*, en_trantype* trantype, F32 dt, void*);
static S32 flexCB(xGoal* rawgoal, void*, en_trantype* trantype, F32 dt, void*);
static S32 rumbleCB(xGoal* rawgoal, void*, en_trantype* trantype, F32 dt, void*);
...
void zNPCB_SB1::scan_cronies()
{
    ...
    for (S32 i = 0; i < count; i++)
    {
        ...
        if (type == 0x4E544234)
        {
            this->plankton = (zNPCBPlankton*)npc;
            return;
        }
    }
}
...
void zNPCB_SB1::SelfSetup()
{
    ...
    // New goals
    goal = psy->AddGoal(NPC_GOAL_BOSSSB1RUMBLESTOMPLOOP, NULL);
    goal->SetCallbacks(rumbleStompLoopCB, NULL, NULL, NULL);

    goal = psy->AddGoal(NPC_GOAL_BOSSSB1FLEX, NULL);
    goal->SetCallbacks(flexCB, NULL, NULL, NULL);

    goal = psy->AddGoal(NPC_GOAL_BOSSSB1RUMBLE, NULL);
    goal->SetCallbacks(rumbleCB, NULL, NULL, NULL);

    psy->BrainEnd();
    
    scan_cronies();

    // Fight starts with the rumble/stomp loop
    psy->SetSafety(NPC_GOAL_BOSSSB1RUMBLESTOMPLOOP);
}
...
void zNPCB_SB1::Reset()
{
    ...
    {
        // Fight always starts fresh with the rumble/stomp loop
        this->psy_instinct->GoalSet(NPC_GOAL_BOSSSB1RUMBLESTOMPLOOP, 0);
    }
    
    if (this->plankton != NULL)
{
    this->plankton->vanish();
    this->plankton->psy_instinct->GoalSet(NPC_GOAL_BPLANKTONAMBUSH, 1);
    }
}
...
U32 zNPCB_SB1::AnimPick(S32 gid, en_NPC_GOAL_SPOT gspot, xGoal* goal)
{
    ...
    {
        // RumbleStompLoop itself is not animated - it dispatches to Rumble or Stomp
        index = Idle01;
        break;
    }
...
{
    // Stop all attacking and force the boss into the flex goal
    this->attacking = 0;
    ...
    if (this->psy_instinct)
    {
        this->psy_instinct->GoalSet(NPC_GOAL_BOSSSB1FLEX, 0);
    }
    
    if (this->plankton != NULL)
        this->plankton->sickum();
}

void zNPCB_SB1::ThanksImDone()
{
    // Resume attacking and leave the flex goal, advancing the fight loop
    this->attacking = 1;
    this->attack_delay = 0.0f;
    this->m_flexing = false;
    this->m_rumbleStompTimer = 0.0f;
    this->m_loopCount++;
...
// RumbleStompLoop - randomly picks Rumble or Stomp for 16 seconds.
// After 16s: first and third completions go to Flex, second completion goes to original Idle.
// The loop count is tracked on the boss so external code and ThanksImDone can advance it.
static S32 rumbleStompLoopCB(xGoal* rawgoal, void*, en_trantype* trantype, F32 dt, void*)
{
    zNPCGoalBossSB1RumbleStompLoop* loop = (zNPCGoalBossSB1RumbleStompLoop*)rawgoal;
    zNPCB_SB1* sb1 = (zNPCB_SB1*)rawgoal->GetOwner();

    S32 nextgoal = 0;

    // Tick the shared timer on the boss
    sb1->m_rumbleStompTimer += dt;

    if (sb1->m_rumbleStompTimer >= 16.0f)
    {
        // 16 seconds up - decide where to go based on how many loops have run
        sb1->m_rumbleStompTimer = 0.0f;

        if (sb1->m_loopCount == 1)
        {
            // Second loop done - hand off to original Idle so full AI takes over
            nextgoal = NPC_GOAL_BOSSSB1IDLE;
            *trantype = GOAL_TRAN_SET;
        }
        else
        {
            // First loop (opening) and third+ loop (ending) - go to Flex
            // HoldUpDude is called here so external code can also call it independently
            sb1->HoldUpDude();
            // HoldUpDude sets the goal itself, so just return 0
            return 0;
        }
    ...
    // Still within the 16s window - dispatch randomly to Rumble or Stomp
    // Only pick a new attack once the previous one finishes (timeInGoal resets on re-entry)
    if (loop->timeInGoal < dt)
    {
        ...
        {
            nextgoal = NPC_GOAL_BOSSSB1STOMP;
        }
        *trantype = GOAL_TRAN_SET;
    }
...
// Flex - boss plays flex anim, does not face player, stays here until m_flexing is cleared
static S32 flexCB(xGoal* rawgoal, void*, en_trantype* trantype, F32 dt, void*)
{
    zNPCB_SB1* sb1 = (zNPCB_SB1*)rawgoal->GetOwner();

    // ThanksImDone clears m_flexing and sets the goal itself, so nothing to do here
    // except stay put - return 0 every frame until externally kicked out
    return 0;
}

// Rumble - plays AttackRumble anim, returns to RumbleStompLoop when done
// Rock-fall hookup goes here later
static S32 rumbleCB(xGoal* rawgoal, void*, en_trantype* trantype, F32 dt, void*)
{
    zNPCGoalBossSB1Rumble* rumble = (zNPCGoalBossSB1Rumble*)rawgoal;
    zNPCB_SB1* sb1 = (zNPCB_SB1*)rawgoal->GetOwner();

    S32 nextgoal = 0;

    rumble->timeInGoal += dt;

    SB1_FaceTarget(sb1, (xVec3*)&globals.player.ent.model->Mat->pos, dt);

    if (sb1->AnimTimeRemain(NULL) < 1.1f * dt)
    {
        nextgoal = NPC_GOAL_BOSSSB1RUMBLESTOMPLOOP;
        *trantype = GOAL_TRAN_SET;
    }
...
// --- Original goal callbacks - unchanged ---
...
static S32 stompCB(xGoal* rawgoal, void*, en_trantype* trantype, F32 dt, void*)
{
    ...
    if (stomp->timeInGoal > 1.5f)
    {
        ...
        if (dist > 90.25f)
        {
            if (sb1->AnimTimeRemain(NULL) < 1.1f * dt)
            {
                // Return to RumbleStompLoop if we're still in the loop window,
                // otherwise back to Idle for the original AI section
                if (sb1->m_rumbleStompTimer < 16.0f && sb1->m_loopCount != 1)
                {
                    nextgoal = NPC_GOAL_BOSSSB1RUMBLESTOMPLOOP;
                }
                else
...
static S32 smashCB(xGoal* rawgoal, void*, en_trantype* trantype, F32 dt, void*)
{
    ...
    {
        // In the ending loop (loopCount >= 3) smash goes back into another rumble/stomp loop
        if (sb1->m_loopCount >= 3)
        {
            sb1->m_rumbleStompTimer = 0.0f;
            nextgoal = NPC_GOAL_BOSSSB1RUMBLESTOMPLOOP;
        }
...
static S32 deflateCB(xGoal* rawgoal, void*, en_trantype* trantype, F32 dt, void*)
{
    ...
    if (sb1->AnimTimeRemain(NULL) < 1.1f * dt)
    {
        if (sSB1_deflated[0] && sSB1_deflated[1])
        {
            sb1->HoldUpDude(); // advances loopCount toward phase 4
        }
        else
        {
            nextgoal = NPC_GOAL_BOSSSB1IDLE;
...
// --- Goal Enter/Process/Exit implementations ---
...
// New goal Enter/Process implementations

S32 zNPCGoalBossSB1RumbleStompLoop::Enter(F32 dt, void* updCtxt)
{
    zNPCB_SB1* sb1 = (zNPCB_SB1*)this->GetOwner();
    this->timeInGoal = 0.0f;
    this->lastAttack = -1;
...
S32 zNPCGoalBossSB1RumbleStompLoop::Process(en_trantype* trantype, F32 dt, void* ctxt, xScene* scene)
{
    // Actual logic is in rumbleStompLoopCB above via the callback system,
    // this just ticks the base goal
    return xGoal::Process(trantype, dt, ctxt, scene);
}

S32 zNPCGoalBossSB1Flex::Enter(F32 dt, void* updCtxt)
{
    zNPCB_SB1* sb1 = (zNPCB_SB1*)this->GetOwner();
    this->timeInGoal = 0.0f;
    // Stop moving, don't face the player
    xVec3Init(&sb1->frame->vel, 0.0f, 0.0f, 0.0f);
    return this->zNPCGoalCommon::Enter(dt, updCtxt);
}

S32 zNPCGoalBossSB1Flex::Process(en_trantype* trantype, F32 dt, void* ctxt, xScene* scene)
{
    this->timeInGoal += dt;
    // Deliberately does not call SB1_FaceTarget - boss looks forward, ignores player
    return xGoal::Process(trantype, dt, ctxt, scene);
}

S32 zNPCGoalBossSB1Rumble::Enter(F32 dt, void* updCtxt)
{
    zNPCB_SB1* sb1 = (zNPCB_SB1*)this->GetOwner();
    this->timeInGoal = 0.0f;
    return this->zNPCGoalCommon::Enter(dt, updCtxt);
...
S32 zNPCGoalBossSB1Rumble::Process(en_trantype* trantype, F32 dt, void* ctxt, xScene* scene)
{
    zNPCB_SB1* sb1 = (zNPCB_SB1*)this->GetOwner();
